...
public class GetMyLeaveApplicationsQueryHandler : IRequestHandler<GetMyLeaveApplicationsQuery, List<LeaveApplicationListDto>>
{
    private readonly ILeaveDbContext _context;
    private readonly IMapper _mapper;
    private readonly ICurrentUserService _currentUserService;

    public GetMyLeaveApplicationsQueryHandler(ILeaveDbContext context, IMapper mapper, ICurrentUserService currentUserService)
    {
        _context = context;
        _mapper = mapper;
    ...
    public async Task<List<LeaveApplicationListDto>> Handle(GetMyLeaveApplicationsQuery request, CancellationToken cancellationToken)
    {
        var tenantId = request.TenantId ?? _currentUserService.TenantId ?? Guid.Empty;

        var query = _context.LeaveApplications
            .Where(la => la.EmployeeId == request.EmployeeId && la.TenantId == tenantId);
