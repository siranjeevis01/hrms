...
using HRMS.Services.Organization.Application.Commands.UpdateDesignation;
...
public class DesignationController : ControllerBase
{
    ...
    [HttpPut("{id:guid}")]
    [ProducesResponseType(typeof(Application.DTOs.DesignationDto), StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    public async Task<IActionResult> Update(Guid id, [FromBody] UpdateDesignationCommand command, CancellationToken cancellationToken)
    {
        if (id != command.Id)
