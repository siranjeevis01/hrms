...
using HRMS.Services.Organization.Application.Commands.UpdateShift;
...
public class ShiftController : ControllerBase
{
    ...
    [HttpPut("{id:guid}")]
    [ProducesResponseType(typeof(Application.DTOs.ShiftDto), StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    public async Task<IActionResult> Update(Guid id, [FromBody] UpdateShiftCommand command, CancellationToken cancellationToken)
    {
        if (id != command.Id)
